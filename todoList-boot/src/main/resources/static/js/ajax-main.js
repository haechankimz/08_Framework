...
function getTotalCount(){ // 함수 정의
  ...
  .then( response => {
    ...
    // response.text() : 응답 데이터를 문자열/숫자 형태로 변환한 결과를 
    //                   가지는 Promise 객체 반환
  ...
  // == 첫 번쨰 then에서 반환된 Promise 객체의 promiseResult 값
  .then(result => {
...
function getCompleteCount(){
  ...
  // 첫 번째 then의 response : 응답 결과, 요청 주소, 응답 데이터 등이 담겨 있음
...
const selectTodoList = () => {
  ...
  .then(result => { //result == 첫 번째 then에서 반환된 결과 값
    ...
    // - JS Object 타입을 String 형태으 JSON데이터로 변환

    const todoList = JSON.parse(result);

    console.log(todoList); // 객체 배열형태 확인
