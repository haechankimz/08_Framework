    ...
    for(let comment of commentList){
      ...
      if(comment.commentDelFl == 'Y') commentRow.innerText = "삭제된 댓글 입니다";
        ...
        if(comment.profileImg == null)
         ...
         profileImg.src = comment.profileImg; // 프로필 이미지
        ...
        // 작성자 역영(commentWriter) 프로필, 닉네임, 날짜 추가
        ...
        // 버튼 영역
        const commenBtnArea = document.createElement("div");
        commenBtnArea.classList.add("comment-btn-area");
        ...
        // 버튼 영역에 답글 추가
        commenBtnArea.append(childCommentBtn);

        // 로그인 한 회원 번호가 댓글 작성자 번호와 같을 때
        // 댓글 수정/삭제 버튼 출력
        if(loginMemberNo != null && loginMemberNo == comment.memberNo){
          ...
          // 버튼 영역에 수정 , 삭제 버튼 추가
          commenBtnArea.append(updateBtn, deleteBtn);
        }

        // 행에 버튼 영역 추가
        commentRow.append(commenBtnArea);
      }
...
const updateComment = (commentNo, btn) => {
  ...
  // 유효성 검사
  if(textarea.value.trim().length == 0){
    alert("댓글 작성 완료 후 수정 버튼을 클릭해 주세요.");
    textarea.focus();
    return;
  }
