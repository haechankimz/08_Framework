...
if(updateInfo != null){
  ...
  updateInfo.addEventListener("submit", e => {
    ...
    // 중복 검사는 나중에 추가 예정 (테스트 시 닉네임 중복 안되게 조심)
    ...
    const result2 = !(addr0 || addr1 || addr2); // 모두 다 입력 O
...
if(profile != null){
  ...
  const changeImageFn = e => {
    ...
    if(file == undefined){
      ...
      // 화면에 추가된 백업본(backupInput)에는 이벤트 리스너가 존재하지 않기 때문에 추가
    ...
    reader.addEventListener("load", e => {
      ...
      // 프로필 이미지 (img 태그)에 src속성으로 url값 세팅
...
/* [input type="file" 사용 시 유의 사항]
  ...
  1. 파일 선택 후 취소를 누르면 선택한 파일이 사라진다
    (value == '')
