...
memberEmail.addEventListener("input", e => {
  ...
  // (알맞은 이메일 형태가 아닌 경우)
  if(!regExp.test(inputEmail)){ // 아닌 경우
  ...
  .then( count => {
    ...
    checkObj.memberEmail = true; // 중복은 유효하지 않음
  } )

  .catch( e=> {
    // fetch() 수행 중 예외 발생시 처리
    console.log(e); // 발갱한 예외(e) 출력
  })
...
sendAuthKeyBtn.addEventListener("click", ()=>{
  ...
  authKeyMessage.classList.remove('error', 'confirm'); // 검정 글씨
  ...
  authTimer =  setInterval(() => {
    ...
    if(min ==0 && sec ==0){
      ...
      authKeyMessage.classList.add('error'); // 글씨 빨간색
