...
// #loginForm이 화면이 존재할 때 (== 로그인 상태 아닐 때)
if(loginForm != null){
...
selectMemberList.addEventListener("click", () => {
  ...
  // 1) 비동기로 회원 목록 조회 (포함될 회원 정보 : 회원번호, 이메일, 닉네임, 탈퇴여부)
  fetch("/member/selectMember")

  // 첫번째 then(response => response.json()) -> 
  // JSON Array -> JS 객체 배열로 변환 [{}, {}, {}, {}]
  
  // 2) 두 번쨰 then tbody에 이미 작성되어 있던 내용(이전에 조회한 목록) 삭제
  .then(response => response.json())
  
  // 3) 두 번째 then에서 조회된 JS 객체 배열을 이용해 tbody에 들어갈 요소를 만들고 값 세팅 후 추가
  .then( memList => {
