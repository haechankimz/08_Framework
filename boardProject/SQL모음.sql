...
DELETE FROM "MEMBER"
WHERE MEMBER_EMAIL = 'member04@kh.or.kr';
...
CREATE TABLE "TB_AUTH_KEY"(
	...
	"CRATE_TIME" DATE DEFAULT SYSDATE NOT NULL
);

COMMENT ON COLUMN "TB_AUTH_KEY"."KEY_NO" IS '인증키 구분번호(시퀀스)';
COMMENT ON COLUMN "TB_AUTH_KEY"."EMAIL" IS '인증 이메일';
COMMENT ON COLUMN "TB_AUTH_KEY"."AUTH_KEY" IS '인증번호';
COMMENT ON COLUMN "TB_AUTH_KEY"."CRATE_TIME" IS '인증번호 생성 시간';
...
UPDATE "MEMBER" SET 
MEMBER_ADDRESS = NULL
WHERE MEMBER_ADDRESS = '06321^^^서울시^^^';
...
COMMENT ON COLUMN "MEMBER"."MEMBER_EMAIL" IS '회원이메일(ID 역할)';
...
CREATE TABLE "BOARD" (
	...
	"BOARD-TITLE"	NVARCHAR2(100)		NOT NULL,
...
COMMENT ON COLUMN "BOARD"."BOARD-TITLE" IS '게시글 제목';
...
COMMENT ON COLUMN "BOARD"."MEMBER_NO" IS '작성한 회원 번호(PK)';
...
COMMENT ON COLUMN "COMMENT"."PARENT_COMMENT_NO" IS '부모 댓글번호(PK)';
...
BEGIN
	FOR I IN 1..2000 LOOP
		...
		VALUES(
			...
			SEQ_COMMENT_NO.CURRVAL || '번 째 댓글 입니다',
			DEFAULT, DEFAULT,
...
-- 번호 / 제목[댓글 개수] / 작성자닉네임 / 작성일/ 좋아요 개수/ 조회수
...
COMMENT ON COLUMN "BOOK"."BOOK_TITLE" IS '첵 제목';
...
INSERT INTO "BOOK"
VALUES(SEQ_BOOK_NO.NEXTVAL, '김해찬', '김해찬', 50000, DEFAULT);
...
INSERT INTO BOARD_IMG
VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본3.jpg', 'test1.jpg',
	2, 1999
);
...
VALUES(1, 1999);  -- 1번 회원이 1999번 글에 좋아요를 클릭함
...
------------------------------------------------------------
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 1',
			DEFAULT, DEFAULT,4021, 1,NULL);
		
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 2',
			DEFAULT, DEFAULT,4021, 1,NULL);
		
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 3',
			DEFAULT, DEFAULT,4021, 1,NULL);

----------------------------------------------------
		
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식 1',
			DEFAULT, DEFAULT, 4021, 1, 6004);
		
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식 2',
			DEFAULT, DEFAULT, 4021, 1, 6004);
		
		
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식 1의 자식 1',
			DEFAULT, DEFAULT, 4021, 1, 6009);
...
/* 계층형 쿼리 */
-- PARENT_NO 값이 NULL인 행이 부모(Lv.1)
START WITH PARENT_COMMENT_NO IS NULL 

-- 부모의 COMMENT_NO와 같은 PARNET_COMMENT_NO 가진 행을 자식으로 지정
CONNECT BY PRIOR COMMENT_NO = PARENT_COMMENT_NO
